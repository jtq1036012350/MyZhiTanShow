...
public class StringUtils {
    ...
    private static final String DEFAULT_DATETIME_PATTERN_24 = "yyyy-MM-dd HH:mm:ss";
    ...
    //24小时
    public static String format24DateTime(long date) {
        return formatDate(new Date(date), DEFAULT_DATETIME_PATTERN_24);
    }
...
//    }
    ...
    /**
     * 讲含有"Date("和"+"的字符串转化为一个时间戳，再将其转化为"yyyy-MM-dd HH:mm:ss"的格式
     *
     * @param s
     * @return
     */
    public static String getDateConvertTime(String s, boolean isneed) {
        if (s != null && s.contains("Date(") && s.contains("+")) {
            String newTime = s.substring(s.indexOf("(") + 1, s.indexOf("+"));
            SimpleDateFormat df;
            if (isneed) {
            ...
            return df.format(Long.parseLong(newTime));
        }
        return "暂无";
    }

    /**
     * 讲含有"Date("和"+"的字符串转化为一个时间戳，再将其转化为"yyyyMMdd HH:mm:ss"的格式
     *
     * @param s
     * @return
     */
    public static String getDateConvertTimeNoLine(String s, boolean isneed) {
        if (s != null && s.contains("Date(") && s.contains("+")) {
            String newTime = s.substring(s.indexOf("(") + 1, s.indexOf("+"));
            SimpleDateFormat df;
            if (isneed) {
    ...
    /**
     * 讲字符串格式化为：date（xxxxxxxxxxxxxxxxxxxxx +800）的字符串
     *
     * @return
     */
    public static String getConvertToDateTime() {
